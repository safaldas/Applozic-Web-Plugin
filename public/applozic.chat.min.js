...
function MckUtils(){var _this=this;var TEXT_NODE=3,ELEMENT_NODE=1,TAGS_BLOCK=["p","div","pre","form"];_this.init=function(){$applozic.ajax({url:MCK_CONTEXTPATH?MCK_CONTEXTPATH+"/v2/tab/initialize.page":"https://apps.applozic.com/v2/tab/initialize.page",contentType:"application/json",type:"OPTIONS"}).done(function(data){});$applozic.ajax({url:MCK_CONTEXTPATH?MCK_CONTEXTPATH+"/rest/ws/message/list":"https://apps.applozic.com/rest/ws/message/list",contentType:"application/json",type:"OPTIONS"}).done(function(data){})};
...
" B)";return""};_this.getFileurl=function(msg){if(typeof msg.fileMeta==="object")if(msg.fileMeta.hasOwnProperty("url"))if(msg.fileMeta.hasOwnProperty("thumbnailBlobKey")){var fileUrl;_this.cloudupdate(msg.fileMeta.blobKey,function(result){fileUrl=result});return fileUrl}else return""+msg.fileMeta.url;else if(msg.fileMeta.thumbnailUrl==="thumbnail_"+msg.fileMeta.name)return MCK_STORAGE_URL+"/files/"+msg.fileMeta.name;else return MCK_FILE_URL+FILE_PREVIEW_URL+msg.fileMeta.blobKey;return""};_this.cloudupdate=
